...
export class BuyerComponent implements OnInit {
    ...
    constructor(private formBuilder: FormBuilder, private http: HttpClient, private sharedService: SharedService, private confirmationService: ConfirmationService, private messageService: MessageService) {
        ...
        this.updateForm = this.buyerForm
    }
    ...
    deleteBuyer(buyer: BuyerModel) {
        ...
        this.confirmationService.confirm({
            ...
            header: 'Confirmation',
            accept: () => {
                this.http.post<MessageModel>(this.sharedService.apiUrl + "buyermaster/delete-single-buyer", buyer).subscribe(result => {
                    this.message = result;
                    if (this.message.status == 200) {
    ...
    confirmUpdate() {
        this.confirmationService.confirm({
            ...
            header: 'Confirmation',
            accept: () => {
                this.http.post<MessageModel>(this.sharedService.apiUrl + "buyermaster/edit-buyer", this.updateForm.value).subscribe(result => {
                    this.message = result;
                    if (this.message.status == 200) {
                        this.messageService.add({severity: 'success', summary: 'Updated', detail: this.message.message});
                        this.fetchBuyerData();
    ...
    onSelectAllChange($event: MouseEvent) {
    ...
    deleteMultipleBuyer() {
        var ids = this.selectedBuyers.map(buyer => buyer.plantId).join(', ');
        this.confirmationService.confirm({

            message: 'Do you wish to delete buyers ' + ids,
            header: 'Confirmation',
            accept: () => {
                this.http.post<MessageModel>(this.sharedService.apiUrl + "buyermaster/delete-multiple-buyer", this.selectedBuyers).subscribe(result => {
                    this.message = result;
                    if (this.message.status == 200) {
